...
public final class ServerItemEssences extends ItemEssences {
    ...
    public void load() {
        ...
        final var path = ServerAlchemy.server.getWorldPath(ServerAlchemy.ALCHEMY_DIR).resolve("item_essences.json");
        this.load(ServerAlchemy.server.registryAccess(), path);
        final var anyDeleted = this.deleteInvalidHolders(ServerAlchemy.server);
        final var anyGenerated = this.generateMissingHolders(ServerAlchemy.server);
    ...
    public void regenerate() {
        if (ServerAlchemy.server == null) return;
        this.holders.clear();
        this.generateMissingHolders(ServerAlchemy.server);
        this.save();
    }
    ...
    private void load(RegistryAccess access, Path path) {
        ...
        ServerAlchemy.tryLoad(path)
                .ifPresent(element -> Packed.CODEC.decode(registryOps, element)
                        ...
                        .ifError(error -> {
                            ServerAlchemy.LOG.error("Failed to decode item essences");
    ...
    private void save(RegistryAccess access, Path path) {
        ...
        Packed.CODEC.encodeStart(registryOps, this.pack())
                ...
                .ifError(error -> {
                    ServerAlchemy.LOG.error("Failed to encode item essences");
                    ServerAlchemy.LOG.error(error.message());
                });
    }

    private boolean deleteInvalidHolders(MinecraftServer server) {
        var total = 0;

        for (var item : BuiltInRegistries.ITEM) {
            if (!this.holders.containsKey(item)) continue;
    ...
    private boolean generateMissingHolders(MinecraftServer server) {
        ...
        for (var item : BuiltInRegistries.ITEM) {
            if (this.holders.containsKey(item)) continue;
            if (!this.canHoldEssences(item)) continue;
            this.holders.put(item, findPreset(server.registryAccess(), item)
                    .orElseGet(() -> generateHolder(
    ...
    private Optional<ItemEssenceHolder> findPreset(RegistryAccess access, Item item) {
    ...
    private ItemEssenceHolder generateHolder(RegistryAccess access, RandomSource random) {
        ...
        var holders = access.registry(ElixirumRegistries.ESSENCE)
                ...
                .orElse(List.of());
        if (holders.isEmpty()) return ItemEssenceHolder.EMPTY;
        ...
        return ItemEssenceHolder.create(essences);
    }
