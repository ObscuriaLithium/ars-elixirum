...
public final class ElixirProcessor {
    private final List<ElixirProcessor.Ingredient> ingredients = Lists.newArrayList();

    public static ElixirContents brew(HolderGetter<Essence> getter, ElixirRecipe recipe) {
        return new ElixirProcessor(getter, recipe).brew();
    }

    private ElixirProcessor(HolderGetter<Essence> getter, ElixirRecipe recipe) {
    ...
    private ElixirContents brew() {
        ...
        essences.forEach((key, value) -> builder.addEffect(ElixirEffect.byWeight(key, value, value)));
