...
public class ElixirOverview extends AbstractWidget {
    ...
    private final Selection<CachedElixir> selection;
    private long lastUpdate = 0;

    public ElixirOverview(Selection<CachedElixir> selection, int x, int y) {
        this(selection, new Signal0(), x, y);
    }

    public ElixirOverview(
            Selection<CachedElixir> selection,
            Signal0 styleChanged, int x, int y
    ) {
        super(x, y, 0, 0, CommonComponents.EMPTY);
        this.selection = selection;
        this.selection.addListener(it -> onUpdate());
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        GuiToolkit.draw(graphics, Textures.SHINE_X256, -128, -256, 256, 256);
